...
drop procedure get_errors_by_os;
delimiter |
create procedure get_errors_by_os( IN os_name varchar(200))
begin
...
CALL get_errors_by_os('Windows 10');



#vtori opit

drop procedure get_errors_by_osy;

delimiter |
create procedure get_errors_by_osy(IN os_name varchar(200))
begin
  declare finished int;
  declare error_id int;
  declare steps_to_reproduce1 text;
  ...
  declare cur cursor for select id, steps_to_reproduce, operating_system, logs 
  from error
  where operating_system = os_name;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET finished = 1;

  drop table if exists temp_results;
  create temporary table temp_results (
    id int,
    steps_to_reproduce VARCHAR(200),
